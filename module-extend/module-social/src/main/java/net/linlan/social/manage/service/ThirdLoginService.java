...
import net.linlan.social.manage.security.ThirdAuthenticationToken;
import net.linlan.social.manage.security.ThirdLogin;
...
public class ThirdLoginService {
    ...
    public AppLoginInfo socialLogin(ThirdLoginBody loginBody) {
        ...
        try {
            ...
            thirdLogin.setOpenType(loginBody.getPlatformType());
            thirdLogin.setCode(loginBody.getCode());
            thirdLogin.setState(loginBody.getState());
            log.debug("开始处理第三方认证: {}", loginBody);
            authentication = authenticationManager
                .authenticate(new ThirdAuthenticationToken(thirdLogin));
        } catch (BadCredentialsException e) {
            log.error("第三方认证处理异常: {}", e.getMessage(), e);
            throw new SupportException("第三方授权登录失败");
        }
        log.debug("开始处理第三方认证后的日志返回记录: {}", loginBody);
        ...
        AsyncManager.me()
            .execute(AsyncFactory.saveAdminLoginLog(loginUser.getUserId(), loginUser.getUsername(),
                Constants.LOGIN_SUCCESS, MessageUtils.message("user.third.login.success"),
                loginUser.getAppId(), loginUser.getUserLid(), LogCategoryEnum.MEMBER.getKey()));
        // 生成token
        AppLoginInfo appLoginInfo = tokenService.createToken(loginUser);
        //单独保存会员用户TOKEN
        thirdMemberTokenService.saveOrUpdateFromThird(appLoginInfo, loginUser);
        return appLoginInfo;
    }
