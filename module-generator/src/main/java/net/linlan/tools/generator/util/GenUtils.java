...
public class GenUtils {
    ...
    public static void initTable(GenTable genTable, String operName) {
        if (genTable.getTableId() == null) {
            genTable.setTableId(RandomUtils.randomLid());
        }
        ...
        genTable.setCreatorId(operName);
    }
    ...
    public static void initColumnField(GenTableColumn column, GenTable table) {
        ...
        if (column.getColumnId() == null) {
            column.setColumnId(RandomUtils.randomLid());
        }
        ...
        // 设置java字段名，主键固定设置为id,前端好使用通用方法传值
        if ("1".equals(column.getIsPk())) {
            column.setJavaField("id");
        } else {
            column.setJavaField(StringUtils.toCamelCase(columnName.toLowerCase(Locale.ROOT)));
        }
        // 设置默认类型
    ...
    public static String convertClassName(String tableName) {
        ...
        if (autoRemovePre && ObjectUtils.isNotEmpty(tablePrefix)) {
    ...
    public static String replaceText(String text) {
        return RegexUtils.replaceAll(text, "(?:表|一体化管理平台)", "");
    }
    ...
    public static String getDbType(String columnType) {
        if (StringUtils.indexOfIgnoreCase(columnType, "(") > 0) {
